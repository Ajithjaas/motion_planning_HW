        ...
        if k == node.h:
            ...
            for neighbor in node_neighbors:
                if neighbor.tag == "NEW" or neighbor.parent == node and neighbor.h != node.h + self.cost(neighbor,node):
                    ...
                    if neighbor.parent != node and neighbor.h > (node.h + self.cost(neighbor,node)):
                        self.insert(node,node.h)
                    else:
                        if neighbor.parent != node and node.h > (neighbor.h + self.cost(neighbor,node)) and neighbor.tag =="CLOSED" and neighbor.h > k:
